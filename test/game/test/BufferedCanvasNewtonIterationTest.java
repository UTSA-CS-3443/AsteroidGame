...
public class BufferedCanvasNewtonIterationTest {
	...
	public static void newtonIterate(int sliceIndex, int threadCount, boolean print) {
		...
		for (int iteration = 0; iteration < iterations; iteration++) {
			...
			if (Math.abs(value - target) <= requiredPrecision) return; //test passed.
			double derivative = commonFactor * 8.0D / Math.PI;
			double correction = (value - target) / derivative;
			result -= correction;
			if (print) {
				System.out.println("Iteration " + iteration + ": " + correction + " -> " + result);
			}
			else if (!(result >= 0.0D && result <= 1.0D)) {
				newtonIterate(sliceIndex, threadCount, true);
				fail("Result not in expected interval: " + result);
			}
		}
		if (!print) newtonIterate(sliceIndex, threadCount, true);
		fail("More than " + iterations + " iterations required for result within " + requiredPrecision + " of correct answer");
	}
}